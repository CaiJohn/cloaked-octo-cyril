...
let string_of_mOOL_expr (e:mOOL_exp):string =
  ...
  match e with
    ...
    | CastExp (e,t) ->
    	"(" ^ string_of_mOOL_type t ^ ")" ^ helper_func e
	| TypedExp (e,t) -> 
		(helper_func e) ^":" 
		^ (string_of_mOOL_type t)
  in helper_func e
...
(* displace a MOOL method modifier *)
...
let string_of_md_decl (m:md_decl) : string = 
	...
	in
	let methodHeader = 
		print_tab() ^ modifier ^ (string_of_mOOL_type m.rettype) 
...
let string_of_attr_decl ((m,(t,id)):attr_decl) : string = 
	print_tab() ^ (string_of_modifier m) ^" "^(string_of_mOOL_type t) 
	^ " " ^ string_of_var_id id	
