...
let process prog  =
    begin
      (*print_endlinev (Jlite_structs.string_of_jlite_program prog);*)
        let typedprog= if useTA then (Jlite_annotatedtyping.type_check_jlite_program prog)
                       else (Jlite_simple_annotatedtyping.type_check_jlite_program prog) in
      (*print_endlinev (Jlite_structs.string_of_jlite_program typedprog);*)
        let ir3prog = (Jlite_toir3.jlite_program_to_IR3 typedprog) in
	print_endlinev (Ir3_structs.string_of_ir3_program ir3prog);
        ...
        begin
          ...
          print_endlinev (Ir3_structs.string_of_ir3_program ir3prog_optimized)
        end
        else print_endlinev "";
        let ir3prog_filtered = Ir3_filter.ir3_program_filter ir3prog_optimized in
      (*print_endlinev (Ir3_structs.string_of_ir3_program ir3prog_filtered);*)
...
in let usage_msg = "==============Jlite compiler=============="
in 
