...
type func_type = var_id * jlite_type * jlite_type list

(* Get the type list of var_decl list *)
let get_tlist (vlist: var_decl list) : jlite_type list =
...
let check_mtd_vars cname mtd = 
  ...
  let occurence (_,c_id) = 
    ...
    then failwith ("\nTypeChecking error - class " ^ cname ^ "\n" 
                   ^ " in method " ^ string_of_var_id mtd.jliteid 
                   ^ " dulplicate local variable: " ^ string_of_var_id c_id ^ "\n") 
    else ()
  in ignore (List.map occurence localvars) 

let match_types (type1: jlite_type) (type2: jlite_type) : bool = 
...
let match_type_list (lst1: jlite_type list) (lst2: jlite_type list) : bool =
  try
...
let check_mtd_in_htbl mtd_sub_htbl cname mtd = 
  if (Hashtbl.mem mtd_sub_htbl mtd.jliteid) then 
    let func_list = Hashtbl.find_all mtd_sub_htbl mtd.jliteid in 
    ...
    let add_cls_into_htbl ((cname,_,mtd_list): class_decl) =
      ...
      let rec itr_mtd_list (mlist: md_decl list) (idx:int) =
        ...
        | mtd::tail ->
          ...
          Hashtbl.add mtd_sub_htbl mtd.jliteid (mtd.ir3id,mtd.rettype,get_tlist mtd.params);
...
let make_t_htbls ((cmain,clist): jlite_program) =
...
let exists_class_decl ((cm,clst): jlite_program) 
                      (cid: class_name) : bool =
  List.exists 
...
*) 
let rec type_check_var_decl_list (p: jlite_program) (vlst: var_decl list) : (bool * string) =
  ...
  in match (helper vlst) with
     | [] -> (true,"")
     | lst -> (false, ("Undefined types: " ^ (string_of_list lst string_of_jlite_type ",")))
 
(* Type check an expression 
   Return the type of the expression 
   and a new TypedExpession 
*) 
let rec type_check_expr (p: jlite_program) 
                        (env: var_decl list) 
                        (classid: class_name) 
                        (exp: jlite_exp) : (jlite_type * jlite_exp) =
  let rec helper (e: jlite_exp) : (jlite_type * jlite_exp) =
    match e with
    ...
    | UnaryExp (op, op_e) ->
      ...
      if ((String.compare (string_of_jlite_op op)) "!" == 0) then 
        match expr_type with 
        | BoolT -> (BoolT, TypedExp (UnaryExp (op, exprnew), BoolT)) 
        | _ -> failwith ("\nType-check error in " ^ classid ^ "." 
                         ^ ". expression fails:\n" 
			 ^ string_of_jlite_expr op_e ^ "Expected of type BoolT, but of type: " 
                         ^ string_of_jlite_type expr_type ^ "\n")
      else if ((String.compare (string_of_jlite_op op) "-") == 0) then
        match expr_type with 
	| IntT -> (IntT, TypedExp (UnaryExp (op, exprnew), IntT))
        | _ -> failwith ("\nType-check error in " ^ classid ^ "." 
			 ^ ". expression fails:\n" 
			 ^ string_of_jlite_expr op_e ^ "Expected of type IntT, but of type: " 
			 ^ string_of_jlite_type expr_type ^ "\n")
      else failwith ("\nType-check error in " ^ classid ^ "." 
                     ^ ". expression fails:\n" 
                     ^ string_of_jlite_expr op_e ^ "Unrecognized unary operator: "
      ...
      if (match_types l_expr_type r_expr_type) then
        ...
        | BooleanOp s -> 
          begin 
            ...
            | _ -> failwith ("\nType-check error in " ^ classid ^ "." 
                             ^ ". expression fails:\n" 
                             ^ string_of_jlite_expr e ^ "Expected of type BoolT, but of type: " 
			     ^ string_of_jlite_type l_expr_type ^ "\n")
          end   
        | RelationalOp "<"
        | RelationalOp "<=" 
        | RelationalOp ">"
	    ...
	    | IntT -> (BoolT, TypedExp (BinaryExp (op,l_expr_new,r_expr_new), BoolT))
            | _ -> failwith ("\nType-check error in " ^ classid ^ "." 
                             ^ ". expression fails:\n" 
                             ^ string_of_jlite_expr e ^ "Expected of type IntT, but of type: " 
                             ^ string_of_jlite_type l_expr_type ^ "\n")
          end
        | RelationalOp "=="
        | RelationalOp "!=" ->
        ...
        | _ -> failwith ("\nType-check error in " ^ classid ^ "." 
                         ^ ". expression fails:\n" 
                         ^ string_of_jlite_expr e ^ "Unsupported binary expression: " 
                         ^ string_of_jlite_type l_expr_type ^ "\n")
      else failwith ("\nType-check error in " ^ classid ^ "." 
                     ^ ". expression fails:\n" 
                     ^ string_of_jlite_expr e
            ...
            if (Hashtbl.mem var_sub_htbl id) then
              let var_type = Hashtbl.find var_sub_htbl id in 
                (var_type, TypedExp (FieldAccess (expr_new,id), var_type))
                           ...
                           ^ ". expression fails:\n" 
                           ^ string_of_jlite_expr obj_e ^ "'s field " 
                           ^ string_of_var_id id ^ " cannot be found\n")
          else failwith ("\nType-check error in " ^ classid ^ "." 
                         ^ ". expression fails:\n" 
                         ^ string_of_jlite_expr obj_e ^ " cannot be found\n")
        | _ -> failwith ("\nType-check error in " ^ classid ^ "." 
                         ^ ". expression fails:\n" 
                         ^ string_of_jlite_expr obj_e ^ "is not ObjectT \n")
      end
    | MdCall (func_e, param_exp_list) ->
      let rec itr_func_sig_list sig_list matched_list matchedNum = 
        match sig_list with
        | (anno_id,ret_type,param_types)::tail ->
          let filter_param_type_exp (param: jlite_exp) : jlite_type * jlite_exp =
            type_check_expr p env classid param in
            let filter_param_exp ((t,exp): (jlite_type * jlite_exp)) : jlite_exp = exp in
              let filter_param_type ((t,exp): (jlite_type * jlite_exp)) : jlite_type = t in
        ...
        | [] -> 
          ...
          then failwith ("\nType-check error in " ^ classid ^ "." 
                         ^ ". expression fails:\n" 
                         ^ string_of_jlite_expr e ^ " arguments type not matched! \n")
      ...
      in begin 
        ...
        | Var varId ->
	      ...
	      (ret_type, TypedExp (MdCall (Var anno_id, args_exp_list), ret_type))               
                            ...
                            ^ ". expression fails:\n" 
                            ^ string_of_jlite_expr func_e ^ " can not find such function \n")
        ...
        | _ -> failwith ("\nType-check error in " ^ classid ^ "." 
                          ^ ". expression fails:\n" 
                          ^ string_of_jlite_expr func_e ^ " is not a function form \n")
      end
    | _ -> (Unknown, e) 
  in helper exp
...
*)
let rec type_check_stmts (p: jlite_program) 
                         (env: var_decl list) 
                         (classid: class_name) 
                         (mthd: md_decl) 
                         (stmtlst: jlite_stmt list)
                         (rettype: jlite_type option) : (jlite_type option * jlite_stmt list) =
  match stmtlst with
  | [] -> (rettype, [])
  | s::tail_lst -> 
    let rec helper (s: jlite_stmt) : (jlite_type option * jlite_stmt) =
      match s with
      | ReturnStmt e ->  
        ...
        then (Some expr_type, ReturnStmt exprnew)
                       ...
                       ^ string_of_jlite_stmt s ^ " - Expect return type "
                       ^ string_of_jlite_type mthd.rettype
                       ^ " but actual return type is " ^ string_of_jlite_type expr_type ^ "\n")
      | ReturnVoidStmt -> 
      ...
      | AssignStmt (id,e) -> 
        if ((exists_var_id env id) == true) then
          ...
          if (match_types var_type expr_type) then (None, AssignStmt (typId,exprnew)) 
                         ...
                         ^ string_of_var_id mthd.jliteid ^ "id: "  
      ...
      | AssignFieldStmt (left_exp, right_exp) ->
        ...
        if (match_types left_exp_type right_exp_type) then
          (None, AssignFieldStmt (new_left_exp, new_right_exp))
                       ...
                       ^ string_of_var_id mthd.jliteid ^ "left exp type and right exp type not match"  
                       ^ ".Statement fails:\n" 
                       ^ string_of_jlite_stmt s ^ "\n")
...
let type_check_mthd_decl (p: jlite_program) 
                         (env: var_decl list)
                         (cname: class_name)
                         (m: md_decl) : md_decl = 
