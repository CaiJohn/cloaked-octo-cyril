...
let can_up_cast_to_type
	(p:jlite_program) (son:jlite_type) (father:jlite_type) =
	if compare_jlite_types son father
	then true
	else
...
let rec type_check_stmts 
	...
	| s::tail_lst -> 
		...
		| AssignStmt (id,e) ->  
			...
			if (compare_jlite_types idtype exprtype) || (can_up_cast_to_type p idtype exprtype)
				...
				let rec helper mthdlst =
					...
					| m::tail_rest -> 
						(type_check_mthd_decl p env cname m)
							::( helper tail_rest)
				in (cname,cparent,cvars,( helper cmthds))
	in 
	begin
