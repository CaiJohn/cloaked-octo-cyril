...
let iR3Expr_to_id3 (exp:ir3_exp) (typ:mOOL_type) 
...
let rec mOOLexpr_to_IR3Expr
	(classid: class_name) 
	(jexp:mOOL_exp) (toidc3:bool) (toid3:bool)
	:(ir3_exp * var_decl3 list * ir3_stmt list) =
	let rec helper 
	...
	match exp with
		...
		| FieldAccess (e,id) -> 
			let (expIR3, vars, stmts) = 
				(mOOLexpr_to_IR3Expr classid e true true) in
		...
		| s::tail_lst -> 
			...
			| IfStmt (e, stmts1, stmts2) -> 
				...
				let (thenvars,thenstmst) = 
					(mOOLstmts_to_IR3Stmts classid mthd stmts1) in 
				let (elsevars,elsestmts) = 
					(mOOLstmts_to_IR3Stmts classid mthd stmts2) in
			...
			| WhileStmt (e, stmts) -> 
				...
				let (expr3,exprvars,exprstmts) = 
					(mOOLexpr_to_IR3Expr classid negatedexp false false) in
				let (vars,stmst) = 
					(mOOLstmts_to_IR3Stmts classid mthd stmts) in 
			...
			| AssignFieldStmt (id,e) ->  
				let (idIR3,idvars,idstmts) = 
					(mOOLexpr_to_IR3Expr classid id false false) in 
				let (expr3,exprvars,exprstmts) = 
					(mOOLexpr_to_IR3Expr classid e false false) in 
			...
			| ReadStmt id -> 
				let (idir3,idvars,idstmts) = 
					(mOOLvarid_to_IR3Expr classid id true) in 	
				let readIR3 = (ReadStmt3 (iR3Expr_get_id3 idir3)) in 
				(idvars,idstmts@[readIR3])
			| PrintStmt e ->  
				let (expr3,exprvars,exprstmts) = 
					(mOOLexpr_to_IR3Expr classid e true false) in 
		  ...
		  let (tailvars,tailstmts) = 
			(mOOLstmts_to_IR3Stmts classid mthd tail_lst) in
		 (vars@tailvars,stmts@tailstmts)
  
(* Transform a method to IR3 *) 
let mOOL_mddecl_to_IR3 cname m  = 
	let (newvars,newstmts) = 
		(mOOLstmts_to_IR3Stmts cname m m.stmts)
	in { id3= string_of_var_id m.ir3id;
		 rettype3=m.rettype; 
		 params3=(ObjectT cname, "this")::
			(mOOLvar_decl_lst_to_ID3 m.params);
		 localvars3=
			(mOOLvar_decl_lst_to_ID3 m.localvars)@newvars; 
		 ir3stmts=newstmts;
		}
