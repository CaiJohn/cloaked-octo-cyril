...
let exists_class_decl 
      ((cm,clst): mOOL_program) (cid:class_name) =
  let rec helper clst =
    match clst with
    | [] -> false 
...
let can_up_cast_to
      ((cm,clst): mOOL_program) (son:class_name) (father:class_name) =
  let rec helper from =
...
let can_cast_to
      (p:mOOL_program) (son:class_name) (father:class_name) =
  (can_down_cast_to p father son)||(can_up_cast_to p son father)
;;

let can_up_cast_to_type
      (p:mOOL_program) (son:mOOL_type) (father:mOOL_type) =
  if compare_mOOL_types son father
  then true
  else
    match son, father with
    | ObjectT s, ObjectT f ->
       can_up_cast_to p s f
    | _,_ -> false
;;
...
(*       ((vt,vid):var_decl) (t:mOOL_type) = *)
(*   (compare_mOOL_types vt t) *)

let can_cast_real_para_to_formal_para
      (p:mOOL_program)  (t:mOOL_type) ((vt,vid):var_decl)=
  can_up_cast_to_type p t vt
;;
     ...
     let rec helper mthdlst =
       ...
       | [] -> failwith 
		 ("Cannot find method:" 
		  ...
		  ^ " in class:" ^ calleecls )
       | mthd::tail_lst -> 
	  if (compare_var_ids mthd.mOOLid calleeid) 
	  then 
	    (
	      match mthd.modifier with
	      | Public ->
		 if ((List.length mthd.params) 
		     == (List.length calleetypes)) 
		 then if (List.for_all2 
			    (can_cast_real_para_to_formal_para p)
			    calleetypes mthd.params
			 ) 
		      then (mthd.ir3id,mthd.rettype)
		      else ( helper tail_lst)
		 else ( helper tail_lst)
	      | Private ->
		 if (String.compare insidecls calleecls)=0
...
let rec type_check_var_decl_list
	  ...
	  (vlst: var_decl list) =
  let rec helper 
	    (vlst: var_decl list) :mOOL_type list =
    match vlst with
    | [] -> []
    | (typ,vid)::tail_lst -> 
       match typ with
  ...
  in match ( helper vlst) with
	...
	begin
     ...
     | lst -> (false, ("Undefined types: " 
		       ^ (string_of_list lst string_of_mOOL_type ",")))
...
let equal_md_param_signatures 
  ...
  then (List.for_all2 
	  (fun (t1,v1) (t2,v2) -> compare_mOOL_types t1 t2) 
	  md1lst md2lst
       )
  else false
...
let rec type_check_md_decl_list
  ...
  let rec helper 
    ...
    | md::tail_lst -> 
       ...
       if (List.for_all
	     (fun md1 -> 
	      not ((compare_var_ids md.mOOLid md1.mOOLid) && 
		     (equal_md_param_signatures 
			md.params md1.params)
		  )
	     ) tail_lst) 
       then helper tail_lst (count +1)
       else md.mOOLid::helper tail_lst (count +1)
...
let rec type_check_expr 
	  (p: mOOL_program)(env: var_decl list) 
	  (classid: class_name) (exp:mOOL_exp) = 
  let rec helper e 
	  : (mOOL_type * mOOL_exp) =
    match e with
    ...
    | BinaryExp (op,arg1,arg2) -> 
	 ...
	 | RelationalOp "==",ObjectT t1,ObjectT "null" 
	 | RelationalOp "==",ObjectT "null", ObjectT t1
	 | RelationalOp "!=",ObjectT t1,ObjectT "null" 
	 | RelationalOp "!=",ObjectT "null", ObjectT t1
	   -> (BoolT, TypedExp(
			  BinaryExp (op, arg1TypedExp, arg2TypedExp), BoolT))
    ...
    | CastExp (e,casttype)->
       ...
       if compare_mOOL_types etype casttype
...
and type_check_method_call 
      ...
      ((calleeexp,calleeparams)
       :mOOL_exp * (mOOL_exp list)) = 
  let (calleeid, calleecls, caleeTypedExp,inside_class) =
    match calleeexp with
    | Var v -> (v, classid, calleeexp,classid)
    | FieldAccess (e,id) -> 
       let (typ, objTypedExp) = 
	 ...
	 match typ with
	 | ObjectT c-> 
	    (id, c, (FieldAccess(objTypedExp,id)),classid)
	 | _ -> failwith 
...
let rec type_check_stmts 
	  ...
	  (rettype: mOOL_type option) 
	: (mOOL_type option *(mOOL_stmt list))  =
  match stmtlst with
  | [] -> (rettype,[])
  | s::tail_lst -> 
     let rec helper s 
	     : (mOOL_stmt * mOOL_type option) =
       match s with
	  ...
	  begin
	    ...
	    | BoolT -> 
		 ...
		 | Some t1, Some t2 -> 
		    ...
		    then failwith 
			   ("\nType-check error in " 
			    ^ classid ^ "." ^ string_of_var_id mthd.mOOLid 
	  ...
	  if (compare_mOOL_types idtype exprtype) || (can_up_cast_to_type p exprtype idtype)
	  then (AssignStmt(scopedid,exprnew),None)
	  else failwith 
		 ("\nType-check error in " 
		  ^ classid ^ "." ^ string_of_var_id mthd.mOOLid 
		  ^ ". Assignment statement fails:\n" 
		  ^ string_of_mOOL_stmt s ^ string_of_mOOL_type exprtype ^ "\n")
       | ReadStmt id -> 
	  let (idtype,scopedid) = (find_var_decl_type env id) in
	  begin
	    match idtype with
	 ...
	 | Some (ObjectT t1), (ObjectT t2) -> 
	    ...
	    then failwith 
		   ("\nType-check error in " ^ cname ^ "." 
		    ...
		    ^ string_of_mOOL_type m.rettype ^ t1 ^ ". \n")
	    else true
	 | Some t1, t2 -> 
	    if (t1!= t2) 
	    then failwith 
...
let type_check_mOOL_program  
  ...
  let rec type_check_class_decl 
	    ((cname,cparent,cattrs,cmthds):class_decl) =
    (* TypeCheck field declarations *)
    let (attrs,cvars) = List.split cattrs in
    let (retval, errmsg) = 
    ...
    (* TypeCheck method declarations *)
	 ...
	 let rec helper mthdlst =
	   ...
	   | m::tail_rest -> 
	      ...
	      ::( helper tail_rest)
	 in (cname,cparent,List.combine attrs cvars,( helper cmthds))
  in 
  begin
    let (mainclass, classes) = p in 
    (* TypeCheck methods overloading *)
    List.iter (fun (cname,cparent,cvars,cmthds) -> 
	       let (retval, errmsg) = 
