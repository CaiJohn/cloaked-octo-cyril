...
type ir3_type = MOOL_structs.mOOL_type

(* Represent both constant and variables *)		  
type idc3 = 
  | IntLiteral3 of int
...
let string_of_pair pair fun1 fun2 =
  match pair with
  | (a,b) -> "("^(fun1 a)^","^(fun2 b)^")"
;;
...
let string_of_ir3_op (e:mOOL_op):string =
  match e with
...
let string_of_class3
  ...
  in
  let attr = 
    (string_of_list cls.var_table string_of_var_decl3 ";\n")
